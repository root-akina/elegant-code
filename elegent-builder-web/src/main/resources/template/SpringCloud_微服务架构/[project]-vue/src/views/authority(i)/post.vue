    ...
    <el-container>
      ...
      <el-main>
        <div class="filter-container">
          <el-input
            v-model="listQuery.postNo"
            :placeholder="$t('table.postNo')"
            style="width: 200px;"
            class="filter-item"
        ...
        >
          ...
          <el-table-column :label="$t('table.postNo')"  prop="postNo" min-width="80px" />
          <el-table-column :label="$t('table.postName')"  prop="postName" min-width="80px" />
          <el-table-column :label="$t('table.deptName')" prop="deptVo.deptName" min-width="80px" />
          <el-table-column :label="$t('table.sortNo')" prop="sortNo" width="100px" />
          <el-table-column :label="$t('table.dataState')" prop="dataState" width="100" :formatter="formatDataState"/>
          <el-table-column :label="$t('table.createTime')" prop="createTime" width="160px" />
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%" top="10px">
          ...
          >
            ...
            <el-form-item :label="$t('table.parentDeptName')" prop="deptNo"
                          :rules="{ required: true, message: '选择部门', trigger: 'change' }" >
              <tree-select v-if="temp.id!=''" :disabled ="true" @input="createPostNo(temp.deptNo)" v-model="temp.deptNo" :data="treeQuery.treeList"></tree-select>
              <tree-select v-if="temp.id==''" :disabled ="false" @input="createPostNo(temp.deptNo)" v-model="temp.deptNo" :data="treeQuery.treeList"></tree-select>
            </el-form-item>
            <el-form-item :label="$t('table.postName')" prop="postName">
              <el-input v-model="temp.postName" :placeholder="$t('table.postName')" />
            </el-form-item>
...
import { createPostNo,fetchList,createInfo, updateInfo} from '@/api/authority/post'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        postNo: [{ required: true, message: '必填', trigger: 'blur' }],
  ...
  methods: {
    ...
    createPostNo(op){
      createPostNo(op).then(response => {
        this.temp.postNo=response.data;
      })
    },
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        console.log('查询职位信息返回:' + response.data);
